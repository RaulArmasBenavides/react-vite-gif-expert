...
const Header = () => {
  ...
  const isOnline = false; // UseOnline();
  ...
  const searchQueryHandler = (event) => {
    if (
      ...
      // navigate(`/searchResult/${searchQuery}`); // Removed
  ...
  // const { pathname } = useLocation(); // Removed
  // const pageName = pathname?.split("/")?.filter(Boolean)?.[0]; // Removed
  const pageName = "home"; // Placeholder for the removed code
    ...
    <div className="flex flex-row h-14 px-4 md:px-5 justify-between items-center sticky top-0 z-20 bg-white dark:bg-black shadow-md">
      ...
      <div className="flex items-center">
        ...
        <div className="flex h-8 w-8 overflow-hidden rounded-full md:ml-4">
          <a href="https://github.com/thecodophile">
            <img
              src="https://avatars.githubusercontent.com/u/20383126?v=4"
              alt="Codophile"
            />
          </a>
        </div>
